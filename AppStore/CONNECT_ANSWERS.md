...
- A bundled snapshot of the FDA National Drug Code Directory
  (`Meds/Resources/NDCDirectory.txt`, from 1.1), public domain, trimmed to
  product name, brand, strength, and dosage form keyed by NDC. It turns the
  code a label prints, or a package barcode carries, into the product it names,
...
- Purpose strings: `NSHealthShareUsageDescription` is the one the person can ever see. The app never writes to Health and never calls share authorization, but App Store Connect's upload validation (ITMS-90683, September 13, 2026) refuses any binary that carries the HealthKit entitlement without `NSHealthUpdateUsageDescription` as well, so that key is declared with a string that says plainly the app never writes to Health. It reads two things through the one per-object authorization: the medications ticked in Health's picker, and the dose events logged against them, both at import and on every launch afterwards for a medication that is also in the app. HealthKit refuses a type-level read request for the dose-event type, so none is made.
