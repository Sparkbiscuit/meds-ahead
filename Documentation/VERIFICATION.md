...
- **Who it's for.** A "Who takes this" field; Today, Supply and the shared
  ...
  unassigned last; the Medications list shows "For" and the name, and searches it.
...
the forecast card read "Ready for pickup"; Edit Medication took
a person's name, a hospital pharmacy, its phone number and an Rx number, and
the detail screen then showed the Pharmacy card with the Rx number in large
type and a Call button naming the pharmacy; Today led with "A refill is on
