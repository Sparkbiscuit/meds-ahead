...
- Consolidated time-slot notifications for simultaneous medications, privacy-safe `Taken` and `Skip` actions for single-dose alerts, and refill-to-Supply routing; a course's reminder at a time it shares with an ongoing medication rings on its own beside it, so neither goes quiet (1.2)
...
- Courses: a last day for a medication taken for a set time, such as an antibiotic, set in the editor. Reminders stop after it; Supply, the detail screen and the widget say whether the supply is enough to finish the course; Today offers to archive it for three days after; a course whose supply ran out before its last day is never called finished; and taking one up again starts it from today (1.2)
- Reminders for a course, or for a schedule that starts later, planned a day at a time for the coming week, with a notice on Today when the reminders planned so far end within three days and the app needs opening to plan more (1.2)
- An optional follow-up, off until chosen in Settings: a second reminder 30 minutes after a dose that isn't logged on this phone, worded to check first in case someone else gave it (1.2)
- A weekly count check: at most once a week, a reminder at 10:00 to count the one medication whose run-out date most needs a count, and the same question as a quick-count card on Today, which asks for missed doses to be logged first (1.2)
- "Why this date?" on every forecast: the count, refills, doses and assumed doses behind the run-out date, what the schedule uses, and when the low-supply alert will come (1.2)
- Save and Scan Next: after saving a scanned bottle the scanner opens again for the next, with a tally of the bottles saved in the session (1.2)
- "Already in Meds Ahead": a bottle of something already tracked is offered as a refill of it rather than saved as a second medication, telling immediate- and extended-release products apart by their codes and brands (1.2)
